...
# Redução ~25% + ajuste de camadas de referência (dobro da espessura)
LOGO_HEIGHT = 135
MAP_HEIGHT  = 675
...
def inject_css() -> None:
    ...
    st.markdown(
        ...
        /* selectboxes 2x mais largos (combobox + popover) */
...
def classify_auto6(series: pd.Series) -> Tuple[pd.Series, List[Tuple[int,int]], List[str]]:
    ...
    if use_quantile:
        edges = _quantile_edges(v.to_numpy(), 6); idx = pd.cut(s, bins=[edges[0]-1e-9] + edges[1:], labels=False, include_lowest=True)
    else:
...
def left_controls() -> Dict[str, Any]:
    ...
    var = st.selectbox(
        ...
        [PLACEHOLDER_VAR,"População (Pessoa/ha)","Densidade demográfica (hab/ha)","Elevação média","Cluster (perfil urbano)","Área de influência de bairro"],
    ...
    if sel_prev and (sel_prev != sel_now): st.cache_data.clear()
...
def collect_reference_overlays() -> List["pdk.Layer"]:
    ...
    if g is not None and not g.empty:
        ...
        layers.append(pdk.Layer(
            ...
            data=gj, filled=True, stroked=True, pickable=False,
            get_fill_color=_hex_to_rgba(REF_GREEN, 220),
            get_line_color=_hex_to_rgba(REF_GREEN, 255),
            get_line_width=1.5, lineWidthUnits="pixels",
        ))
...
def render_pydeck(center: Tuple[float, float],
    ...
    if limite_gdf is not None and not limite_gdf.empty:
        ...
        layers.append(pdk.Layer(
            ...
            get_line_color=[20,20,20,200], get_line_width=1.2, lineWidthUnits="pixels"
        ))
    ...
    if draw_setores_outline:
        ...
        if geoms_only is not None and not geoms_only.empty:
            ...
            layers.append(pdk.Layer(
                ...
                get_line_color=[80,80,80,180], get_line_width=0.8, lineWidthUnits="pixels"
            ))
