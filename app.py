...
def inject_css() -> None:
    st.markdown(
        ...
        <style>
            ...
            .pb-card {{ background:#fff; border:1px solid rgba(20,64,125,.10); box-shadow:0 1px 2px rgba(0,0,0,.04); border-radius:16px; padding:18px; }}
...
def build_controls(key_prefix="main_") -> Tuple[str,str,str,str,bool]:
    ...
    limite = st.selectbox("Limites Administrativos",
                          ...
                          index=0, key=f"{key_prefix}pb_limite",
                          help="Escolha a desagregação espacial para exibir como contorno.")
    variavel = st.selectbox("Variáveis", ["Densidade","Zoneamento"],
                            index=1, key=f"{key_prefix}pb_variavel",
...
def make_satellite_map(center=(-23.55,-46.63), zoom=10, tiles_opacity=0.5):
    ...
    # Esri Imagery (sem rótulos) → evita conflito com nossos tooltips
    folium.TileLayer(
...
def add_centroid_labels(m, gdf, label_col: str, font_px: int = 20):
    ...
    for name, pt in zip(gdf[label_col].astype(str), centers):
        ...
        html = f"<div style='font-size:{font_px}px;font-weight:900;color:#111;text-shadow:0 2px 6px rgba(255,255,255,.9);white-space:nowrap;'>{name}</div>"
        folium.Marker(
            location=[pt.y, pt.x],
...
def plot_density_variable(m, setores_gdf, dens_df):
    ...
    folium.GeoJson(
        ...
        tooltip=folium.features.GeoJsonTooltip(
            ...
            sticky=True, labels=True, class_name="pb-big-tooltip",
            style=("background:#fff;color:#111;font-size:64px;font-weight:800;white-space:nowrap;"
                   "border:1px solid #222;border-radius:8px;padding:10px 14px;"),
            max_width=1200,
        ),
    ...
    with st.container():
        ...
        with map_col:
            if folium is not None and st_folium is not None:
                ...
                inject_leaflet_css(fmap, font_px=900)  # tamanho do tooltip

                if fmap is not None:
                    if gdf_limite is not None:
                        add_admin_outline(fmap, gdf_limite, layer_name=limite, color="#000000", weight=1.2)
                        # rótulos permanentes (opcional)
                        if labels_on:
                            # escolhe coluna de nome automaticamente
                            cols_lower = {c.lower(): c for c in gdf_limite.columns}
                            label_col = None
                            if "ds_nome" in cols_lower: label_col = cols_lower["ds_nome"]
