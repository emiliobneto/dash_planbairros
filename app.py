...
def inject_css() -> None:
    ...
    st.markdown(
        ...
        <style>
            ...
            .pb-title {{ font-size: 2.3rem; line-height: 1.15; font-weight: 700; letter-spacing: .5px; }}
            .pb-subtitle {{ opacity: .95; margin-top: 4px; font-size: 1.1rem; }}

            .pb-card {{
                background:#fff; border:1px solid rgba(20,64,125,.10); box-shadow:0 1px 2px rgba(0,0,0,.04);
                border-radius:16px; padding:16px;
            }}
            /* ===== CONTROLES LATERAIS (mais fortes) ===== */
            .pb-card h4 {{ font-size: 1.8rem !important; margin: 0 0 .5rem 0; }}
            /* rótulos dos selectboxes */
            .pb-card .stSelectbox label, .pb-card label, .pb-card .stMarkdown p {{
                font-size: 1.6rem !important; font-weight: 700 !important;
            }}
            /* área clicável / valor selecionado */
            .pb-card div[role="combobox"] {{
                font-size: 1.45rem !important; min-height: 54px !important;
            }}
            /* opções abertas do dropdown */
            .pb-card [data-baseweb="select"] * {{ font-size: 1.45rem !important; }}
            .pb-card .stSelectbox svg {{ transform: scale(1.5); }}   /* seta maior */
            /* ============================================ */

            .stTabs [data-baseweb="tab-list"] button[role="tab"] {{
                background:transparent; border-bottom:3px solid transparent; font-weight:600;
            }}
...
def _first_parquet_matching(folder: Path, name_candidates: list[str]) -> Optional[Path]:
    ...
    for fp in folder.glob("*.parquet"):
        if _slug(fp.stem) in wanted: return fp
    return None
...
def _safe_read_parquet(path: Path) -> Optional[pd.DataFrame]:
    try:
        if path.stat().st_size < 1024: return None
    except Exception:
        return None
    try:
...
def load_admin_layer(layer_name: str):
    ...
    try:
        ...
        if pdf is None:
            st.warning(f"Falha ao ler {path.name}."); return None
        geom_col = next((c for c in pdf.columns if c.lower() in ("geometry","geom","wkb","wkt")), None)
        if geom_col is None:
            st.warning(f"{path.name}: coluna de geometria não encontrada."); return None
        try:
            ...
            st.warning(f"{path.name}: não foi possível reconstruir geometria ({exc})."); return None
    try:
...
def load_density() -> Optional[pd.DataFrame]:
    ...
    if direct.exists(): search_order.append(direct)
    for base in (DENS_DIR, ADM_DIR):
        if base.exists():
            by_name = _first_parquet_matching(base, DENSITY_CANDIDATES)
            if by_name is not None: search_order.append(by_name)
...
def build_tabs() -> None:
    ...
    for i, aba in enumerate([t1,t2,t3,t4], start=1):
        with aba: st.markdown(f"**Conteúdo da Aba {i}** — espaço para textos/explicações.")
    st.write("")
...
def build_controls(key_prefix: str = "main_") -> Tuple[str, str, str, str]:
    ...
    limite = st.selectbox(
        ...
        index=0, help="Escolha a desagregação espacial para exibir como contorno.",
    ...
    variavel = st.selectbox(
        "Variáveis", ["Densidade","Zoneamento"], index=var_index,
        help="Variáveis agregadas por Setor Censitário a partir do mapa de densidade.",
        key=f"{key_prefix}pb_variavel",
    )
    metrica = st.selectbox("Métricas", ["—"], index=0, help="Reservado para cálculos futuros.", key=f"{key_prefix}pb_metrica")
    info = st.selectbox("Informações", ["—"], index=0, help="Notas/documentação.", key=f"{key_prefix}pb_info")
...
def add_admin_outline(m, gdf, layer_name: str, color="#000000", weight=1.0):
    """Contorno + camada de área para hover com tooltip grande."""
    if gdf is None or folium is None: return
    cols_lower = {c.lower(): c for c in gdf.columns}
    label_col: Optional[str] = None
    lname = layer_name.lower()
    if "distr" in lname and "ds_nome" in cols_lower: label_col = cols_lower["ds_nome"]
    elif "subpref" in lname and "sp_nome" in cols_lower: label_col = cols_lower["sp_nome"]
    elif "setor" in lname:
        for k in ("cd_geocodi","cd_setor"):
            if k in cols_lower: label_col = cols_lower[k]; break
    ...
    folium.GeoJson(
        data=gf_line.to_json(), name=f"{layer_name} (contorno)", pane="vectors",
    ...
    if label_col:
        ...
        folium.GeoJson(
            data=gf_area.to_json(), name=f"{layer_name} (hover)", pane="vectors",
            ...
            tooltip=folium.features.GeoJsonTooltip(
                ...
                style=("background:#fff;color:#111;font-size:64px;font-weight:800;white-space:nowrap;"
...
def plot_density_variable(m, setores_gdf, dens_df):
    ...
    if folium is None or gpd is None or setores_gdf is None or dens_df is None: return
    ...
    for cand in ("CD_GEOCODI","CD_SETOR"):
        if cand.lower() in [c.lower() for c in setores_gdf.columns] and cand.lower() in [c.lower() for c in dens_df.columns]:
            key=cand; break
    if key is None: st.info("Não foi possível identificar a coluna de setor (CD_GEOCODI/CD_SETOR)."); return
    key_g=_real(setores_gdf,key); key_d=_real(dens_df,key); val_d=_real(dens_df,"densidade_hec")
    if val_d not in dens_df.columns: st.info("Coluna `densidade_hec` não encontrada."); return
    ...
    if mg.empty: st.info("Nenhum setor para desenhar após o merge."); return
    try: mg["geometry"] = mg.geometry.simplify(SIMPLIFY_TOL, preserve_topology=True)
    except Exception: pass
    ...
    folium.GeoJson(
        data=mg.to_json(), name="Densidade", pane="vectors",
        style_function=style_fn,
        tooltip=folium.features.GeoJsonTooltip(
            fields=["setor_key","densidade_hec"], aliases=["Setor:","Densidade (hab/ha):"],
...
def bar_for_density(dens_df: Optional[pd.DataFrame]):
    if dens_df is None or dens_df.empty: st.info("Sem dados de densidade para o gráfico."); return
    col = next((c for c in dens_df.columns if c.lower()=="densidade_hec"), None)
    if col is None: st.info("Coluna `densidade_hec` não encontrada."); return
    ...
    fig.update_traces(marker_color=PB_COLORS["laranja"]); fig.update_layout(margin=dict(l=0,r=0,t=0,b=0))
...
def main() -> None:
    ...
    with st.container():
        if pre_show_chart:
            ...
            left, map_col = st.columns([1,6], gap="small"); right=None
