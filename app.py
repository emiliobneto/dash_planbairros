...
# Gradiente alto contraste (sem azul/verde) – usaremos 6 classes (Jenks)
ORANGE_RED_GRAD = ["#fee8c8", "#fdd49e", "#fdbb84", "#fc8d59", "#e34a33", "#b30000"]

SIMPLIFY_TOL = 0.0006  # leve simplificação p/ performance (ajuste fino)
...
def _find_file(folder: Path, candidates: List[str], exts: Tuple[str, ...]) -> Optional[Path]:
    """
    Procura arquivo por stem (case/acentos-insensível) e por extensão.
    """
...
# =============================================================================
# Leitura e saneamento geoespacial
# =============================================================================
...
def _drop_bad_geoms(gdf: "gpd.GeoDataFrame") -> "gpd.GeoDataFrame":
    """
    Remove geometrias None/vazias e tenta corrigir inválidas (buffer(0)).
    Isso é CRÍTICO para evitar KeyError no folium_map.get_bounds().
    """
    ...
    # tenta corrigir inválidas de forma leve
    try:
...
def gdf_to_featurecollection(gdf: "gpd.GeoDataFrame", keep_cols: Optional[List[str]] = None) -> Optional[dict]:
    """
    Converte para dict GeoJSON e remove features sem geometry/coordinates.
    Evita o KeyError de bounds do Folium.
    """
    if gdf is None or gdf.empty:
        return None
...
def build_setores_joined_by_fid() -> Optional["gpd.GeoDataFrame"]:
    ...
    # normaliza fid como inteiro (quando possível)
    g = g_id.copy()
    v = df_vars.copy()
    ...
    v = v.dropna(subset=[fid_vars]).copy()
    v = v.drop_duplicates(subset=[fid_vars])
    ...
    # remove geometry da tabela de atributos se existir
...
def jenks_breaks(values: List[float], k: int) -> Optional[List[float]]:
    """
    Jenks natural breaks (DP). Retorna lista de k+1 breaks (min..max).
    """
    ...
    if len(uniq) <= k:
        # poucas classes possíveis: usa "uniq" como cortes
...
def make_carto_map(center=(-23.55, -46.63), zoom=11):
    ...
    try:
        ...
        folium.map.CustomPane("green", z_index=650).add_to(m)      # áreas verdes (acima de tudo)
    except Exception:
        pass
...
def add_admin_outline(m, gdf, name: str, color="#000000", weight=1.2, show=True):
    """
    Limites como LINHA (boundary). Remove geometrias ruins para evitar KeyError no bounds.
    """
...
def add_green(m, gdf, show=True):
    if folium is None or gdf is None or gdf.empty:
        return
    poly = gdf[["geometry"]].copy()
    poly = _drop_bad_geoms(poly)
    poly = _simplify_safe(poly, SIMPLIFY_TOL)
    ...
    folium.GeoJson(
        data=gj,
        pane="green",
        style_function=lambda f: {"color": PB_COLORS["verde"], "weight": 0.0, "fillOpacity": 1.0, "fillColor": PB_COLORS["verde"]},
    ).add_to(fg)
    fg.add_to(m)
...
def add_lines(m, gdf, name: str, color: str, weight: float, pane: str, show=True):
    if folium is None or gdf is None or gdf.empty:
        return
    ln = gdf[["geometry"]].copy()
    ln = _drop_bad_geoms(ln)
    ln = _simplify_safe(ln, SIMPLIFY_TOL)
...
# =============================================================================
# Pintura – setores (Jenks 6 classes)
# =============================================================================
def paint_setores_jenks(m, setores: "gpd.GeoDataFrame", value_col: str, label: str):
    ...
    df = _simplify_safe(df, SIMPLIFY_TOL)
    if df.empty:
        return
    ...
    # legenda discreta (Jenks)
    if Element is not None:
        items = []
        for i in range(6):
            lo = br[i]
            hi = br[i+1]
            col = ORANGE_RED_GRAD[i]
            items.append((f"{lo:,.2f} – {hi:,.2f}", col))
        ...
        <div style="position:absolute; left: 16px; bottom: 24px; z-index:9999; background:rgba(255,255,255,.95);
          ...
          <div style="font-weight:700; margin-bottom:6px">{label} (Jenks, 6 classes)</div>
          {rows}
        </div>
        """
...
def left_controls() -> Dict[str, Any]:
    ...
    var = st.selectbox(
        ...
        index=1,
        key="pb_var",
        help="Variáveis são lidas de SetoresCensitarios2023 e aplicadas na geometria do IDCenso2023 via join por fid.",
    )
    ...
    show_isocronas = st.checkbox("Isócronas (contorno)", value=False)

    st.markdown("### Referências (sempre acima)")
    ...
    st.caption("Use o botão abaixo para limpar caches de dados em memória.")
...
def main() -> None:
    ...
    with map_col:
        # --- monta setores por join fid
        setores_join = build_setores_joined_by_fid()

        # centro preferencial: distritos -> idcenso -> fallback
        center = (-23.55, -46.63)
        ...
        # --- camadas administrativas (linhas)
        ...
        if ui["show_isocronas"]:
            add_admin_outline(fmap, load_admin("Isócronas"), "Isócronas", color="#111111", weight=0.9, show=True)
        ...
        if setores_join is None or setores_join.empty:
            ...
            # mapeia colunas (robusto p/ nomes como na sua imagem)
            ...
            if var == "População" and col_pop:
                paint_setores_jenks(fmap, setores_join, col_pop, "População")
            elif var == "Densidade demográfica" and col_den:
                paint_setores_jenks(fmap, setores_join, col_den, "Densidade demográfica")
            elif var == "Diferença de elevação" and col_de:
                paint_setores_jenks(fmap, setores_join, col_de, "Diferença de elevação")
            elif var == "Elevação" and col_el:
                paint_setores_jenks(fmap, setores_join, col_el, "Elevação")
            elif var == "Raio máximo de caminhada" and col_ra:
                paint_setores_jenks(fmap, setores_join, col_ra, "Raio máximo de caminhada")
            elif var == "Cluster" and col_cl:
                paint_setores_cluster(fmap, setores_join, col_cl)
            else:
                st.info(
                    "Coluna da variável não encontrada. "
                    "Verifique nomes no SetoresCensitarios2023 (ex.: Populacao, Diferenca_elevacao, elevacao, Cluster, raio_maximo_caminhada)."
                )

        # --- overlays de referência (acima)
        if ui["show_rios"]:
        ...
        # Layer control no canto inferior direito (como você pediu)
        try:
            folium.LayerControl(position="bottomright", collapsed=False).add_to(fmap)
        except Exception:
            pass

        # IMPORTANTÍSSIMO: agora todas as camadas são "sanitizadas" (sem geometry null),
        # então st_folium -> folium_map.get_bounds() não quebra mais.
        st_folium(
            fmap,
            height=780,
