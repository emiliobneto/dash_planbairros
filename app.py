...
# ====================== caminhos e util (padrão anterior) ======================
try:
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                geo = vals.dropna().apply(
                    lambda b: wkb.loads(b, hex=isinstance(b, str))
                )
            return gpd.GeoDataFrame(
                pdf.drop(columns=[geom_col]), geometry=geo, crs=4326
            )
        except Exception:
            return None
...
def load_setores_geom() -> Tuple[Optional["gpd.GeoDataFrame"], Optional[str]]:
    ...
    try:
        ...
        gdfm["geometry"] = gdfm.geometry.simplify(
            SIMPLIFY_M_SETORES, preserve_topology=True
        )
...
def load_metric_column(var_label: str) -> Optional[pd.DataFrame]:
    ...
    wanted = find_col(
        cols, *(cand if isinstance(cand, (list, tuple)) else [cand])
    )
...
def _simplify_overlay_types(
    gdf: "gpd.GeoDataFrame", tol_m: float
) -> "gpd.GeoDataFrame":
    try:
        gm = gdf.to_crs(3857)
    except Exception:
...
def gdf_to_geojson_obj(
    gdf: "gpd.GeoDataFrame", cols: List[str] | Tuple[str, ...]
) -> dict:
...
def classify_auto6(
    series: pd.Series,
) -> Tuple[pd.Series, List[Tuple[int, int]], List[str]]:
    ...
    if vmin == vmax:
        idx = pd.cut(
            s, bins=[vmin - 1e-9, vmax + 1e-9], labels=False, include_lowest=True
        )
        palette = _sample_gradient(ORANGE_RED_GRAD, 6)
        return (
            idx.fillna(-1).astype("Int64"),
            [(int(round(vmin)), int(round(vmax)))],
            palette,
        )
    edges_eq = _equal_edges(vmin, vmax, 6)
    idx_eq = pd.cut(
        s,
        bins=[edges_eq[0] - 1e-9] + edges_eq[1:],
        labels=False,
        include_lowest=True,
    )
    ...
    if use_quantile:
        ...
        idx = pd.cut(
            s, bins=[edges[0] - 1e-9] + edges[1:], labels=False, include_lowest=True
        )
    else:
        edges = edges_eq
        idx = idx_eq
...
def show_numeric_legend(title: str, breaks: List[Tuple[int, int]], palette: List[str]):
    ...
    html = (
        f"<div class='legend-card'><div class='legend-title'>{title}</div>{''.join(rows)}</div>"
    )
...
def make_tile_basemap(style: str) -> "pdk.Layer":
    ...
    * ESRI via services.arcgisonline.com (ordem {z}/{y}/{x})
    * Força crossOrigin=anonymous via loadOptions para evitar bloqueio do canvas
    """
...
def render_pydeck(
    ...
    *,
    ...
    try:
        ph.pydeck_chart(
            deck, use_container_width=True, height=MAP_HEIGHT, key=component_key
        )
    except TypeError:
        ...
        if var == "Área de influência de bairro":
            ...
            if iso_raw is not None and not iso_raw.empty:
                ...
                if cls:
                    ...
                    try:
                        gm = g.to_crs(3857)
                        gm["geometry"] = gm.buffer(0).geometry.simplify(
                            SIMPLIFY_M_ISOCRONAS, preserve_topology=True
                        )
                        g = (
                            gm.to_crs(4326)
                        ...
                        pass
                    g["__rgba__"] = g["__k__"].map(
                        lambda v: _hex_to_rgba(
                            lut_color.get(int(v), "#c8c8c8"), 200
                        )
                    )
                    g["__label__"] = g["__k__"].map(
                        lambda v: f"{int(v)} – {lut_label.get(int(v), 'Outros')}"
                    )
                    gdf_layer = g[["geometry", "__rgba__", "__label__"]]
                    center = center_from_bounds(g)
                    present = sorted(
                        {int(v) for v in g["__k__"].dropna().unique().tolist()}
                    )
                    legend_items = [
                        (f"{k} – {lut_label[k]}", lut_color[k]) for k in present
                    ]
                ...
                if metric is not None:
                    ...
                    if var == "Cluster (perfil urbano)":
                        ...
                        joined["__rgba__"] = s.map(
                            lambda v: _hex_to_rgba(
                                cmap.get(int(v) if pd.notna(v) else -1, "#c8c8c8"), 200
                            )
                        )
                        joined["__label__"] = s.map(
                            lambda v: labels.get(int(v), "Outros")
                            if pd.notna(v)
                            else "Sem dado"
                        )
                        ...
                        joined["__rgba__"] = classes.map(
                            lambda k: color_map.get(
                                int(k) if pd.notna(k) else -1, _hex_to_rgba("#c8c8c8", 200)
                            )
                        )
                        joined["__label__"] = (
                            pd.to_numeric(joined["__value__"], errors="coerce")
                            .round(0)
                            .astype("Int64")
                            .astype(str)
                        )
